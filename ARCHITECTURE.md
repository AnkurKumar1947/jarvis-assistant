# 🏗️ Jarvis Assistant - Architecture

## Overview

Jarvis is a **fully local** AI voice assistant for macOS, built with a modern monorepo structure. It consists of two main applications communicating via WebSocket:

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                           JARVIS ASSISTANT                                  │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   ┌─────────────────────┐          WebSocket           ┌─────────────────┐  │
│   │                     │◄───────────────────────────►│                 │  │
│   │    Next.js Web UI   │         (Socket.io)          │  Express Server │  │
│   │    (Port 3000)      │                              │   (Port 3001)   │  │
│   │                     │          REST API            │                 │  │
│   │                     │◄────────────────────────────►│                 │  │
│   └─────────────────────┘                              └─────────────────┘  │
│            │                                                    │           │
│            │                                                    │           │
│            ▼                                                    ▼           │
│   ┌─────────────────────┐                              ┌─────────────────┐  │
│   │   Browser APIs      │                              │  External APIs  │  │
│   │   • Camera          │                              │  • Ollama LLM   │  │
│   │   • Microphone      │                              │  • macOS System │  │
│   │   • Web Audio       │                              │  • Piper TTS    │  │
│   └─────────────────────┘                              └─────────────────┘  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## System Architecture

### High-Level Data Flow

```
                              USER INPUT
                                  │
                    ┌─────────────┴─────────────┐
                    │                           │
                    ▼                           ▼
            ┌──────────────┐           ┌──────────────┐
            │  Voice Input │           │  Text Input  │
            │  (Browser)   │           │  (Chat Box)  │
            └──────┬───────┘           └──────┬───────┘
                   │                          │
                   └───────────┬──────────────┘
                               │
                               ▼
                    ┌──────────────────┐
                    │   WebSocket Msg  │
                    │   'send_message' │
                    └────────┬─────────┘
                             │
                             ▼
              ┌──────────────────────────────┐
              │     ASSISTANT SERVICE        │
              │  ┌────────────────────────┐  │
              │  │   Intent Parser        │  │
              │  │   (Pattern Matching)   │  │
              │  └───────────┬────────────┘  │
              │              │               │
              │   ┌──────────┴──────────┐    │
              │   │                     │    │
              │   ▼                     ▼    │
              │ ┌─────────┐      ┌──────────┐│
              │ │Commands │      │   LLM    ││
              │ │Registry │      │ (Ollama) ││
              │ └────┬────┘      └────┬─────┘│
              │      │                │      │
              │      └───────┬────────┘      │
              │              │               │
              │              ▼               │
              │  ┌────────────────────────┐  │
              │  │   Speech Synthesizer   │  │
              │  │      (Piper TTS)       │  │
              │  └────────────────────────┘  │
              └──────────────┬───────────────┘
                             │
                             ▼
                    ┌──────────────────┐
                    │   WebSocket Msg  │
                    │    'message'     │
                    └────────┬─────────┘
                             │
                             ▼
                      ┌─────────────┐
                      │   Web UI    │
                      │  (Display)  │
                      └─────────────┘
```

---

## Frontend Architecture (apps/web)

### Tech Stack
- **Framework**: Next.js 14 (App Router)
- **UI**: React 18 + Tailwind CSS + shadcn/ui
- **State**: Zustand stores
- **Real-time**: Socket.io client
- **Styling**: CSS Variables for theming

### Component Structure

```
apps/web/
├── app/                        # Next.js App Router
│   ├── layout.tsx              # Root layout with providers
│   ├── page.tsx                # Main dashboard (entry point)
│   ├── globals.css             # Global styles + theme variables
│   └── providers.tsx           # Context providers wrapper
│
├── components/
│   ├── assistant/              # 🤖 Assistant Panel
│   │   ├── AssistantAvatar.tsx    # Animated avatar (idle/listening/speaking)
│   │   ├── AssistantView.tsx      # Main assistant display
│   │   ├── ControlButtons.tsx     # Voice/mute controls
│   │   ├── StateIndicator.tsx     # Current state badge
│   │   └── Waveform.tsx           # Audio visualization
│   │
│   ├── camera/                 # 📹 Camera Feed
│   │   ├── CameraFeed.tsx         # Live camera display
│   │   └── CameraControls.tsx     # Camera toggle/settings
│   │
│   ├── chat/                   # 💬 Chat Interface
│   │   ├── ChatPanel.tsx          # Main chat container
│   │   ├── ChatInput.tsx          # Message input + send
│   │   ├── MessageList.tsx        # Message history
│   │   ├── MessageBubble.tsx      # Individual message
│   │   └── TypingIndicator.tsx    # "Jarvis is typing..."
│   │
│   ├── metrics/                # 📊 System Metrics
│   │   ├── SystemMetrics.tsx      # Metrics grid
│   │   ├── MetricCard.tsx         # Individual metric
│   │   ├── CPUMeter.tsx           # CPU usage ring
│   │   ├── RAMMeter.tsx           # Memory usage
│   │   ├── DiskMeter.tsx          # Disk usage
│   │   └── BatteryMeter.tsx       # Battery level
│   │
│   ├── settings/               # ⚙️ Settings
│   │   ├── SettingsDialog.tsx     # Settings modal
│   │   └── VoiceSettings.tsx      # TTS voice selector
│   │
│   └── ui/                     # 🎨 Base Components (shadcn)
│       ├── button.tsx
│       ├── card.tsx
│       ├── dialog.tsx
│       └── ...
│
├── hooks/                      # Custom React Hooks
│   ├── useSocket.ts               # WebSocket connection
│   ├── useAssistant.ts            # Assistant state management
│   ├── useAudioRecorder.ts        # Browser audio recording
│   ├── useCamera.ts               # Camera stream access
│   ├── useMetrics.ts              # System metrics subscription
│   └── useTheme.ts                # Theme switching
│
├── stores/                     # Zustand State Stores
│   ├── assistantStore.ts          # Assistant state
│   ├── chatStore.ts               # Chat messages
│   ├── metricsStore.ts            # System metrics
│   └── themeStore.ts              # Theme preferences
│
└── lib/
    ├── socket.ts                  # Socket.io client setup
    ├── constants.ts               # App constants
    └── utils.ts                   # Utility functions
```

### Frontend State Flow

```
┌──────────────────────────────────────────────────────────────────┐
│                        FRONTEND STATE                            │
├──────────────────────────────────────────────────────────────────┤
│                                                                  │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────────┐   │
│  │ assistantStore  │  │   chatStore     │  │  metricsStore   │   │
│  ├─────────────────┤  ├─────────────────┤  ├─────────────────┤   │
│  │ • state         │  │ • messages[]    │  │ • cpu           │   │
│  │ • isListening   │  │ • isTyping      │  │ • ram           │   │
│  │ • voice         │  │ • addMessage()  │  │ • disk          │   │
│  │ • ttsEnabled    │  │ • clearChat()   │  │ • battery       │   │
│  └────────┬────────┘  └────────┬────────┘  └────────┬────────┘   │
│           │                    │                    │            │
│           └────────────────────┼────────────────────┘            │
│                                │                                 │
│                    ┌───────────▼───────────┐                     │
│                    │    Socket.io Client   │                     │
│                    │  (Real-time Updates)  │                     │
│                    └───────────┬───────────┘                     │
│                                │                                 │
└────────────────────────────────┼─────────────────────────────────┘
                                 │
                                 ▼
                          [ Backend Server ]
```

---

## Backend Architecture (apps/server)

### Tech Stack
- **Runtime**: Node.js 18+
- **Framework**: Express.js
- **Real-time**: Socket.io
- **TTS**: Piper (neural voices)
- **LLM**: Ollama (local)
- **Audio**: Sox for recording

### Module Structure

```
apps/server/
├── src/
│   ├── index.ts                # 🚀 Entry point (Express + Socket.io setup)
│   │
│   ├── core/                   # ⚙️ Core Configuration
│   │   ├── config.ts              # Config loader (JSON)
│   │   └── types.ts               # TypeScript interfaces
│   │
│   ├── socket/                 # 🔌 WebSocket Layer
│   │   ├── index.ts               # Socket.io initialization
│   │   ├── handlers.ts            # Event handlers
│   │   └── events.ts              # Event constants
│   │
│   ├── routes/                 # 🛤️ REST API
│   │   └── api.ts                 # HTTP endpoints
│   │
│   ├── services/               # 🧠 Business Logic
│   │   ├── assistantService.ts    # Main assistant orchestrator
│   │   └── metricsService.ts      # System metrics collector
│   │
│   ├── brain/                  # 🤖 Intelligence Layer
│   │   ├── llm.ts                 # Ollama LLM client
│   │   ├── intentParser.ts        # Intent extraction
│   │   └── memory.ts              # Conversation memory
│   │
│   ├── commands/               # ⚡ Command Registry
│   │   ├── index.ts               # Command router
│   │   ├── system.ts              # Volume, brightness, battery
│   │   ├── apps.ts                # Open/close applications
│   │   ├── media.ts               # Music playback control
│   │   └── utilities.ts           # Time, date, calculations
│   │
│   ├── speech/                 # 🔊 Speech Processing
│   │   ├── synthesizer.ts         # TTS orchestrator
│   │   ├── transcriber.ts         # STT (speech-to-text)
│   │   └── providers/
│   │       └── piperProvider.ts   # Piper neural TTS
│   │
│   ├── audio/                  # 🎵 Audio Processing
│   │   ├── index.ts               # Audio exports
│   │   ├── recorder.ts            # Microphone recording
│   │   ├── player.ts              # Audio playback
│   │   └── wakeWord.ts            # Wake word detection
│   │
│   └── utils/                  # 🔧 Utilities
│       ├── logger.ts              # Colored console logging
│       ├── macOS.ts               # macOS AppleScript helpers
│       └── sounds.ts              # System sounds
│
├── config/
│   └── default.json            # Default configuration
│
├── voices/
│   └── piper/                  # Piper voice models (.onnx)
│       ├── en_GB-alan-medium.onnx
│       └── manifest.json
│
└── bin/                        # Binary dependencies
    ├── piper                   # Piper TTS binary
    ├── piper_phonemize         # Phoneme converter
    └── espeak-ng-data/         # Language data
```

### Backend Processing Pipeline

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                        BACKEND PROCESSING PIPELINE                          │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│    User Message                                                             │
│         │                                                                   │
│         ▼                                                                   │
│  ┌─────────────────────────────────────────────────────────────────────┐    │
│  │                     ASSISTANT SERVICE                               │    │
│  │                                                                     │    │
│  │   1. Log user message                                               │    │
│  │   2. Add to conversation memory                                     │    │
│  │   3. Parse intent                                                   │    │
│  │         │                                                           │    │
│  │         ▼                                                           │    │
│  │   ┌──────────────┐      ┌──────────────┐      ┌──────────────┐     │    │
│  │   │   Command    │ YES  │   Execute    │      │   Return     │     │    │
│  │   │   Match?     │─────►│   Command    │─────►│   Result     │     │    │
│  │   └──────┬───────┘      └──────────────┘      └──────────────┘     │    │
│  │          │ NO                                                       │    │
│  │          ▼                                                          │    │
│  │   ┌──────────────┐      ┌──────────────┐      ┌──────────────┐     │    │
│  │   │   LLM        │ YES  │   Query      │      │   Return     │     │    │
│  │   │   Available? │─────►│   Ollama     │─────►│   Response   │     │    │
│  │   └──────┬───────┘      └──────────────┘      └──────────────┘     │    │
│  │          │ NO                                                       │    │
│  │          ▼                                                          │    │
│  │   ┌──────────────┐      ┌──────────────┐                           │    │
│  │   │  Rule-Based  │─────►│   Fallback   │                           │    │
│  │   │  Response    │      │   Response   │                           │    │
│  │   └──────────────┘      └──────────────┘                           │    │
│  │                                                                     │    │
│  │   4. Add response to memory                                         │    │
│  │   5. Emit via WebSocket                                             │    │
│  │   6. Speak via TTS                                                  │    │
│  │                                                                     │    │
│  └─────────────────────────────────────────────────────────────────────┘    │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

---

## Communication Protocol

### WebSocket Events

#### Client → Server
| Event | Payload | Description |
|-------|---------|-------------|
| `send_message` | `{ content: string, source: 'voice' \| 'text' }` | Send user message |
| `start_listening` | - | Begin voice recording |
| `stop_listening` | - | Stop voice recording |
| `get_metrics` | - | Request system metrics |
| `get_state` | - | Request assistant state |
| `get_history` | - | Request message history |
| `clear_history` | - | Clear conversation |

#### Server → Client
| Event | Payload | Description |
|-------|---------|-------------|
| `message` | `Message` object | New message (user/assistant) |
| `assistant_state` | `AssistantState` string | State change notification |
| `metrics_update` | `SystemMetrics` object | Real-time system metrics |
| `history` | `Message[]` array | Conversation history |
| `error` | `{ code, message }` | Error notification |

### REST API Endpoints

| Method | Endpoint | Description |
|--------|----------|-------------|
| GET | `/health` | Server health check |
| GET | `/api/status` | Assistant status |
| GET | `/api/metrics` | System metrics |
| POST | `/api/message` | Send message (alternative) |
| GET | `/api/history` | Message history |
| GET | `/api/voices` | Available TTS voices |
| POST | `/api/voices/test` | Test a voice |
| GET/POST | `/api/settings/voice` | Voice settings |

---

## TTS Architecture

The Text-to-Speech system supports multiple providers with automatic fallback:

```
┌─────────────────────────────────────────────────────────────────┐
│                    SPEECH SYNTHESIZER                           │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   Configuration                                                 │
│   ├─ provider: 'auto' | 'piper' | 'macos'                      │
│   ├─ voice: string                                              │
│   └─ rate: number                                               │
│                                                                 │
│   ┌─────────────────────────────────────────────────────────┐   │
│   │              Provider Selection (auto mode)             │   │
│   │                                                         │   │
│   │   ┌─────────────┐    Available?    ┌─────────────┐     │   │
│   │   │   Piper     │───────YES───────►│   Use       │     │   │
│   │   │  Provider   │                  │   Piper     │     │   │
│   │   └──────┬──────┘                  └─────────────┘     │   │
│   │          │ NO                                           │   │
│   │          ▼                                              │   │
│   │   ┌─────────────┐    Available?    ┌─────────────┐     │   │
│   │   │   macOS     │───────YES───────►│   Use       │     │   │
│   │   │  Provider   │                  │   macOS     │     │   │
│   │   └──────┬──────┘                  └─────────────┘     │   │
│   │          │ NO                                           │   │
│   │          ▼                                              │   │
│   │   ┌─────────────┐                                      │   │
│   │   │  No TTS     │                                      │   │
│   │   │  Available  │                                      │   │
│   │   └─────────────┘                                      │   │
│   └─────────────────────────────────────────────────────────┘   │
│                                                                 │
│   Piper Provider                    macOS Provider             │
│   ├─ Neural network voices          ├─ Native 'say' command    │
│   ├─ ONNX model files               ├─ System voices           │
│   ├─ High quality                   ├─ No setup required       │
│   └─ Requires model download        └─ Lower quality           │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## Command System

Commands are pattern-matched and executed by category:

```
┌─────────────────────────────────────────────────────────────────┐
│                     COMMAND REGISTRY                            │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   ┌─────────────┐   ┌─────────────┐   ┌─────────────┐          │
│   │   SYSTEM    │   │    APPS     │   │   MEDIA     │          │
│   ├─────────────┤   ├─────────────┤   ├─────────────┤          │
│   │ • volume    │   │ • open      │   │ • play      │          │
│   │ • mute      │   │ • close     │   │ • pause     │          │
│   │ • battery   │   │ • list      │   │ • next      │          │
│   │ • brightness│   │ • focus     │   │ • previous  │          │
│   └─────────────┘   └─────────────┘   └─────────────┘          │
│                                                                 │
│   ┌─────────────┐   ┌─────────────┐                            │
│   │  UTILITIES  │   │CONVERSATION │                            │
│   ├─────────────┤   ├─────────────┤                            │
│   │ • time      │   │ • greetings │                            │
│   │ • date      │   │ • help      │                            │
│   │ • calculate │   │ • identity  │                            │
│   │ • search    │   │ • fallback  │                            │
│   └─────────────┘   └─────────────┘                            │
│                                                                 │
│   Pattern Matching:                                             │
│   ┌───────────────────────────────────────────────────────────┐ │
│   │ "set volume to 50" → /set\s+volume\s+(?:to\s+)?(\d+)/     │ │
│   │ "open Safari"      → /open\s+(\w+)/                       │ │
│   │ "what time is it"  → /(?:what|what's|whats).*time/        │ │
│   └───────────────────────────────────────────────────────────┘ │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## State Machine

The assistant operates as a finite state machine:

```
                              ┌──────────────┐
                              │ INITIALIZING │
                              └──────┬───────┘
                                     │ Config loaded
                                     ▼
     ┌──────────────────────────────────────────────────────────┐
     │                                                          │
     │   ┌────────────────────────────────────────────────┐     │
     │   │                                                │     │
     │   │         ┌──────┐                               │     │
     │   │         │      │                               │     │
     │   │         │ IDLE │◄───────────────────────────┐  │     │
     │   │         │      │                            │  │     │
     │   │         └──┬───┘                            │  │     │
     │   │            │ User speaks/types              │  │     │
     │   │            ▼                                │  │     │
     │   │      ┌───────────┐                          │  │     │
     │   │      │ LISTENING │ (voice mode)             │  │     │
     │   │      └─────┬─────┘                          │  │     │
     │   │            │ Audio captured                 │  │     │
     │   │            ▼                                │  │     │
     │   │      ┌────────────┐                         │  │     │
     │   │      │ PROCESSING │ (transcription)         │  │     │
     │   │      └─────┬──────┘                         │  │     │
     │   │            │ Text extracted                 │  │     │
     │   │            ▼                                │  │     │
     │   │      ┌──────────┐                           │  │     │
     │   │      │ THINKING │ (LLM/intent)              │  │     │
     │   │      └────┬─────┘                           │  │     │
     │   │           │                                 │  │     │
     │   │  Command? │                                 │  │     │
     │   │    ┌──────┴──────┐                          │  │     │
     │   │    │             │                          │  │     │
     │   │    ▼             ▼                          │  │     │
     │   │ ┌───────────┐  (skip)                       │  │     │
     │   │ │ EXECUTING │                               │  │     │
     │   │ └─────┬─────┘                               │  │     │
     │   │       │                                     │  │     │
     │   │       └──────────┬──────────────────────────┘  │     │
     │   │                  │ Response ready              │     │
     │   │                  ▼                             │     │
     │   │            ┌──────────┐                        │     │
     │   │            │ SPEAKING │ (TTS output)           │     │
     │   │            └────┬─────┘                        │     │
     │   │                 │ Speech complete              │     │
     │   │                 └──────────────►(back to IDLE) │     │
     │   │                                                │     │
     │   └────────────────────────────────────────────────┘     │
     │                                                          │
     │                        ┌───────┐                         │
     │                        │ ERROR │ (recoverable)           │
     │                        └───┬───┘                         │
     │                            │ Auto-recovery               │
     │                            └──────────►(back to IDLE)    │
     │                                                          │
     └──────────────────────────────────────────────────────────┘
                                     │
                                     │ SIGTERM/SIGINT
                                     ▼
                              ┌──────────┐
                              │ SHUTDOWN │
                              └──────────┘
```

---

## External Dependencies

### Required
| Dependency | Purpose | Installation |
|------------|---------|--------------|
| Node.js 18+ | Runtime | `brew install node` |
| npm | Package manager | (included with Node.js) |

### Optional (Enhanced Features)
| Dependency | Purpose | Installation |
|------------|---------|--------------|
| Ollama | Local LLM | `brew install ollama` |
| Piper | Neural TTS | `brew install piper` |
| Sox | Audio recording | `brew install sox` |
| Piper voices | Voice models | `./scripts/download-piper-voices.sh` |

---

## Configuration

Configuration is loaded from `apps/server/config/default.json`:

```json
{
  "assistant": {
    "name": "Jarvis",
    "wakeWord": "jarvis",
    "language": "en"
  },
  "ollama": {
    "host": "http://localhost:11434",
    "model": "llama3.2:3b",
    "timeout": 60000
  },
  "tts": {
    "provider": "auto",
    "voice": "en_GB-alan-medium",
    "rate": 1.0,
    "enabled": true
  },
  "commands": {
    "enableSystem": true,
    "enableApps": true,
    "enableMedia": true,
    "enableUtilities": true
  }
}
```

---

## Security Considerations

- **Fully Local**: All processing happens on your machine
- **No Cloud APIs**: LLM (Ollama) and TTS (Piper) run locally
- **No Telemetry**: No data sent to external servers
- **macOS Permissions**: Requires microphone and accessibility access for full functionality

---

## Future Architecture (Planned)

```
┌─────────────────────────────────────────────────────────────────┐
│                    FUTURE ENHANCEMENTS                          │
├─────────────────────────────────────────────────────────────────┤
│                                                                 │
│   ┌─────────────────┐                                          │
│   │  Mobile App     │◄────────► [ WebSocket ]                  │
│   │  (React Native) │                                          │
│   └─────────────────┘                                          │
│                                                                 │
│   ┌─────────────────┐                                          │
│   │  Smart Home     │◄────────► [ HomeKit / Home Assistant ]   │
│   │  Integration    │                                          │
│   └─────────────────┘                                          │
│                                                                 │
│   ┌─────────────────┐                                          │
│   │  Camera AI      │◄────────► [ Vision Models / YOLO ]       │
│   │  Processing     │                                          │
│   └─────────────────┘                                          │
│                                                                 │
│   ┌─────────────────┐                                          │
│   │  Plugin System  │◄────────► [ Custom Commands / Skills ]   │
│   │                 │                                          │
│   └─────────────────┘                                          │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## Quick Reference

### Start Development
```bash
# Terminal 1: Backend
cd apps/server && npm run dev

# Terminal 2: Frontend
cd apps/web && npm run dev
```

### Key Files
| Purpose | File |
|---------|------|
| Server entry | `apps/server/src/index.ts` |
| Main orchestrator | `apps/server/src/services/assistantService.ts` |
| WebSocket handlers | `apps/server/src/socket/handlers.ts` |
| Frontend entry | `apps/web/app/page.tsx` |
| Socket client | `apps/web/lib/socket.ts` |
| Type definitions | `apps/server/src/core/types.ts` |
